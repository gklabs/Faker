...
def main():
    ...
    #vectorizations
    train_0, val_0 = getrep(cleaned_train['speaker_job'], cleaned_validation['speaker_job'])
    train_1, val_1 = getrep(cleaned_train['Spoke_at'], cleaned_validation['Spoke_at'])
    train_2, val_2 = getrep(cleaned_train['text'], cleaned_validation['text'])
    
    train_3, val_3 = getdummies(cleaned_train, cleaned_validation)    
    
    traindf = pd.concat([train_0, train_1, train_2, train_3], axis =1)
    validationdf = pd.concat([val_0, val_1, val_2, val_3], axis =1)
    ...
    X = min_max_scaler.fit_transform(traindf.values)
    X_test = min_max_scaler.transform(validationdf.values)
    
    #Logistic
    LRclf = LogisticRegression(solver='liblinear', random_state=1).fit(X, y_train)
    y_lr_pred= LRclf.predict(X_test)
    ...
    #SVM
    svmclf1 = SVC(gamma='auto').fit(X, y_train)
    y_svm_pred= svmclf1.predict(X_test)
        
    #Compute accuracy
    print(accuracy_score(y_test, y_lr_pred),accuracy_score(y_test, y_nb_pred),
    ...
    output= pd.DataFrame(list(zip(newli,y_test,y_svm_pred,y_nb_pred,y_lr_pred)), columns=["text","truth","svm","nb","lr"])
    output.to_csv(path+"baseline.csv")
