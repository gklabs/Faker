...
def getrep(train_text, val_text, rep='tfidf'):
    ...
    if rep == 'tfidf':
        vectorizer = TfidfVectorizer(use_idf = True, analyzer='word', tokenizer=dummy_fun,preprocessor=dummy_fun, token_pattern=None)
    if rep == 'ngramfreq':
    ...
    if rep == 'ngramtfidf':
        vectorizer = TfidfVectorizer(use_idf = True)#analyzer='word', tokenizer=dummy_fun,preprocessor=dummy_fun, token_pattern=None, ngram_range=(1,2))
...
def baseline(X, y_train, X_test, y_test, colnames):
    ...
    #Logistic
    LRclf = LogisticRegression(solver='liblinear').fit(X[colnames], y_train)
    #coef_dict = {}
...
def main():
    ...
    ##Add bleu scores
    #traindf['bleu'] = np.random.randint(0, 1, traindf.shape[0])
    #validationdf['bleu'] = np.random.randint(0, 1, validationdf.shape[0])
    
    ##Add SENTIMENT scores
    #traindf['sentiment'] = np.random.randint(-1, 1, traindf.shape[0])
    #validationdf['sentiment'] = np.random.randint(-1, 1, validationdf.shape[0])
    ...
    print("Logistic regression")
    #Logistic
    LRclf = LogisticRegression(solver='liblinear').fit(X, y_train)
    
    print('--------------------------------------------------------------------')
    print('Labels ', LRclf.classes_)
    for subcoefs in LRclf.coef_:
        sublist = list(zip(subcoefs, traindf.columns))
        print('len of each list ', len(sublist))
        print(sublist)
    print('--------------------------------------------------------------------')
    ...
    d1, d2 = baseline(X, y_train, X_test, y_test, traindf.columns)   
    #output[rep+'_lr'], output[rep+'_nb']
    output.to_csv("baselineResults.csv")
    
    
    #h = pd.DataFrame(list(zip(validation_1.text, y_test, y_lr_pred)), columns = ['text', 'truth', 'Predictions'])
    #h.to_csv('result.csv')
